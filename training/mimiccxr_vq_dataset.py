...
class MimicCxrVqDataset(Dataset):
    def __init__(
        self, report_path: str, vq_path: str, tokenizer_len: int, mode: str, stage: int
    ):
        ...
        self.db = pd.read_csv(
            self.report_path / "mimic-cxr-2.0.0-split.csv",
            index_col="dicom_id",
            dtype=str,
        )
        ...
        if mode == "train":
            ...
            self.db = self.db.loc[
                (self.db["split"] == "test") | (self.db["split"] == "validate")
            ]
        else:
        ...
        with open(
            self.report_path / "mimic-cxr-2.0.0-selected-pa-ap-earlist-study.pickle",
            "rb",
        ) as f:
    ...
    def _check_vq(self):
        ...
        print(
            f"{bcolors.FAIL}[Warning] # of error dicom vq(s): {len(error_ids)}{bcolors.ENDC}"
        )
        return error_ids
...
def get_inject_vq_fun(tokenizer):
    ...
    def inject_vq(input_ids: List[int], cxr_vq_shifted: List[int]) -> List[int]:
        ...
        second_idx = _find_vq_replace_token_idx(
            input_ids[first_idx + 1 :], replace_tokens
        )
...
def get_extract_vq_fun(tokenizer):
    ...
    def extract_vq(input_ids: List[int]):
        sequence = cast(torch.Tensor, input_ids.clone()).flatten().cpu().numpy()
        ...
        if np.any(is_vq):
            ...
            if vq_start >= reponse_start:
                ...
                if len(vq) == CXR_VQ_VQ_LEN:
                    ...
                    print(
                        f"VQ token found but not of length {CXR_VQ_VQ_LEN}: {len(vq)}"
                    )
                    return None
            else:
                return None
